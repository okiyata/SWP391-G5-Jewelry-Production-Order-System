...
const ChatComponent = () => {
    ...
    async function connect(event) {
        ...
        if (enteredUserId) {
            try {
                const saleStaffResponse = await fetch(`https://swp391-g5-jewelry-production-order-system.onrender.com/${enteredUserId}/sale-staff`);
                ...
                const response = await fetch(`https://swp391-g5-jewelry-production-order-system.onrender.com/user/check/${enteredUserId}`);
                if (response.ok) {
    ...
    function onUserFound(user) {
        ...
        const socket = new SockJS('https://swp391-g5-jewelry-production-order-system.onrender.com/ws', null, { withCredentials: true });
    ...
    async function fetchUnreadMessages() {
        try {
            const unreadMessagesResponse = await fetch(`https://swp391-g5-jewelry-production-order-system.onrender.com/unread-messages/${userId}`);
        ...
        if(currentUser) {
            try {
                if (currentUser.role === "CUSTOMER") {
                    ...
                    if (userSaleStaff !== "") {
                        const allUsersResponse = await fetch(`https://swp391-g5-jewelry-production-order-system.onrender.com/user/check/${userSaleStaff}`);
                        const user = await allUsersResponse.json();
                        await renderConnectedUsers([user]);
                    }
                } else {
                    roleSelectListRef.current.classList.remove('hidden');
                    const allUsersResponse = await fetch(`https://swp391-g5-jewelry-production-order-system.onrender.com/users/${roleSelectRef.current.value}`);
    ...
    async function markMessagesAsRead(recipientId) {
        try {
            const response = await fetch(`https://swp391-g5-jewelry-production-order-system.onrender.com/mark-messages-as-read/${recipientId}`, {
                method: 'POST'
            });
            if (response.ok) {
    ...
    async function fetchAndDisplayUserChat() {
        const userChatResponse = await fetch(`https://swp391-g5-jewelry-production-order-system.onrender.com/messages/${userId}/${selectedUserId}`);
    ...
    function onLogout() {
        if (stompClient) {
            stompClient.publish({
                destination: "https://swp391-g5-jewelry-production-order-system.onrender.com/app/user.disconnectUser",
    ...
    async function handleImageUpload(event) {
        ...
        if (imageFile) {
            try {
                ...
                const response = await fetch('https://swp391-g5-jewelry-production-order-system.onrender.com/chat/upload', {
                    method: 'POST',
                    body: formData
                });
